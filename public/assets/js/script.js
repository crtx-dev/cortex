...
$('#setupForm').onsubmit=async e=>{e.preventDefault();$('#authError').textContent='';try{await api('/api/auth/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:$('#setupPassword').value,confirm:$('#setupConfirm').value})});if(!finishAuthentication())await bootAuthenticated()}catch(x){authError(x)}};
$('#loginForm').onsubmit=async e=>{e.preventDefault();$('#authError').textContent='';try{await api('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:$('#loginPassword').value,TOTP:$('#loginTOTP').value})});if(!finishAuthentication())await bootAuthenticated()}catch(x){authError(x)}};
