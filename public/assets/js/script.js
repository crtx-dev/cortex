...
$('#setupForm').onsubmit=async e=>{e.preventDefault();$('#authError').textContent='';try{await api('/api/auth/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({display:$('#setupDisplay').value,username:$('#setupUsername').value,password:$('#setupPassword').value,confirm:$('#setupConfirm').value})});if(!finishAuthentication())await bootAuthenticated()}catch(x){authError(x)}};
